...
const createMeal = async (req: Request, res: Response) => {
  // console.log('Received request for createMeal.'); // Debug log
...
const getMeals = async (req: Request, res: Response) => {
  const userId = req.user?.id;
  // NEW: Get query parameters for search and filters
  ...
  try {
    ...
    // Add search query (q) filter
    if (q) {
      queryText += ` AND (m.title ILIKE $${paramIndex} OR m.description ILIKE $${paramIndex})`;
      queryParams.push(`%${q}%`);
      paramIndex++;
    }
    ...
    // Add tag filter
    if (tag) {
      queryText += ` AND m.id IN (SELECT meal_id FROM meal_tags WHERE tag_name ILIKE $${paramIndex})`;
    ...
    if (minRating !== undefined) {
      queryText += ` AND m.overall_rating >= $${paramIndex}`;
      queryParams.push(parseInt(minRating as string));
      paramIndex++;
    }
    if (maxRating !== undefined) {
      queryText += ` AND m.overall_rating <= $${paramIndex}`;
      queryParams.push(parseInt(maxRating as string));
      paramIndex++;
    }

    // Add date range filter
    if (startDate) {
      queryText += ` AND m.date_made >= $${paramIndex}`;
...
const updateMeal = async (req: Request, res: Response) => {
  ...
  const date_made = req.body.date_made;
  const overall_rating = parseInt(req.body.overall_rating);
  const photo_url_is_null = req.body.photo_url_is_null;
